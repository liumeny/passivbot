...
from telegram_reports import TelegramReportManager
...
class Passivbot:
    ...
    async def update_pnls(self):
        ...
        if self.stop_signal_received:
            return False
        if not hasattr(self, "_pnls_update_lock"):
            self._pnls_update_lock = asyncio.Lock()
        async with self._pnls_update_lock:
    ...
    async def init_telegram_reports(self) -> None:
        """Validate Telegram credentials and prepare the report manager when enabled."""
        if self._telegram_reporter is not None:
            return
        if not bool(get_optional_live_value(self.config, "telegram_reports_enabled", False)):
            return
        credentials_path = require_live_value(self.config, "telegram_credentials_path")
        state_path = Path(f"caches/telegram_reports/{self.exchange}/{self.user}.json")
        self._telegram_reporter = TelegramReportManager.from_paths(
            credentials_path=credentials_path,
            state_path=state_path,
        )
        logging.info(
            "[telegram] live reports enabled | credentials=%s | state=%s",
            credentials_path,
            state_path,
        )

    async def maintain_telegram_reports(self):
        """Check for due Telegram reports and send them when their slots are unsent."""
        await self.init_telegram_reports()
        if self._telegram_reporter is None:
            return
        logging.info("[telegram] starting report scheduler")
        while not self.stop_signal_received:
            try:
                sent = await self._telegram_reporter.maybe_send_due_reports(self)
                if sent:
                    logging.info("[telegram] sent report(s): %s", ", ".join(sent))
                await asyncio.sleep(30.0)
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                logging.error("[telegram] report scheduler error: %s", exc)
                if self.logging_level >= 2:
                    traceback.print_exc()
    ...
    async def start_data_maintainers(self):
        ...
        if self._telegram_reporter is not None:
            maintainer_names.append("maintain_telegram_reports")
        if self.ws_enabled:
