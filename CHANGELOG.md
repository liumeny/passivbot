...
### Added
- **Telegram daily/weekly live reports** - Live bots can now send a Telegram daily report at 22:00 Asia/Shanghai covering the previous 24 hours plus a current position snapshot, and a weekly report every Sunday at 22:00 Asia/Shanghai covering the previous 7 days. Added `live.telegram_reports_enabled` and `live.telegram_credentials_path`, with cross-restart dedup state stored under `caches/telegram_reports/{exchange}/{user}.json`.
...
- **Telegram reports now include Binance total account balance** - Daily/weekly Telegram live reports now add an `account_total_balance` line using Binance wallet-balance data in the configured quote currency, alongside the existing futures `balance_raw` and position snapshot.
- **Telegram balance/report presentation overhaul** - `/balance` now shows Binance total-asset wallet distribution by wallet bucket with amount and percentage breakdown, and daily/weekly Telegram reports now use a formal HTML layout with Chinese section headers, wallet-distribution account overview, and clearer position snapshots.
- **Telegram position views now focus on actionable summaries** - `/status` now shows richer per-position detail (mode, entry/close order counts, mark, price move, notional, uPnL, WE) and marks missing prices as `n/a` instead of `0.0000`. `/positions24h` now collapses fill noise into completed position cycles only, summarizing each closed trade by open/close time, peak size, average prices, realized PnL, and exit type.
...
### Fixed
- **Binance one-way mode live support** - Live Binance bots now detect the account's current position mode instead of forcing hedge mode. In one-way mode they omit `positionSide`, use `reduceOnly` for closes, and infer `LONG/SHORT` from `BOTH` positions/orders/trades so restarts and order reconciliation work without dual-side mode.
- **Non-whitelisted live positions no longer get strategy management** - During live execution, symbols that already have positions but are not in the side's approved whitelist now default to `manual` instead of inheriting graceful-stop management, do not consume whitelist strategy slots, and are skipped by per-symbol exchange config updates.
